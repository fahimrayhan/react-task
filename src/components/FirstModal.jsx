        ...
        <Modal show={modalState.showModal1} size="lg" centered scrollable>
            ...
            <Modal.Body >
                ...
                <table className="table table-striped ">
                    ...
                    <tbody>
                        {data?.filter(item => checkbox === true ? item.id % 2 === 0 : item).filter(item => item.phone.includes(searchQuery) || item.country.name.includes(searchQuery)).map((item, index) => (
                            <tr key={`${item.id}-${index}`} onClick={() => dispatch({ type: 'showModalTwo', value: true })}>
                                <td>{item?.phone}</td>
